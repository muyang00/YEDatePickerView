...
@implementation YEPickerBaseView
...
- (void)layoutSubviews {
    ...
    if (_alertView && self.pickerStyle.topCornerRadius > 0) {
        // 设置顶部圆角
        [YEPickerStyle ye_setView:_alertView roundingCorners:UIRectCornerTopLeft | UIRectCornerTopRight withRadius:self.pickerStyle.topCornerRadius];
    }
}
...
- (UIButton *)cancelBtn {
    if (!_cancelBtn) {
        ...
        // 设置按钮圆角或边框
        if (self.pickerStyle.cancelBorderStyle == YEBorderStyleSolid) {
        ...
        } else if (self.pickerStyle.cancelBorderStyle == YEBorderStyleFill) {
            _cancelBtn.layer.cornerRadius = self.pickerStyle.cancelCornerRadius > 0 ? self.pickerStyle.cancelCornerRadius : 6.0f;
            _cancelBtn.layer.masksToBounds = YES;
        }
    }
...
- (UIButton *)doneBtn {
    if (!_doneBtn) {
        ...
        // 设置按钮圆角或边框
        if (self.pickerStyle.doneBorderStyle == YEBorderStyleSolid) {
        ...
        } else if (self.pickerStyle.doneBorderStyle == YEBorderStyleFill) {
            _doneBtn.layer.cornerRadius = self.pickerStyle.doneCornerRadius > 0 ? self.pickerStyle.doneCornerRadius : 6.0f;
            _doneBtn.layer.masksToBounds = YES;
        }
    }
...
- (YEPickerStyle *)pickerStyle {
    if (!_pickerStyle) {
        _pickerStyle = [[YEPickerStyle alloc]init];
    }
    return _pickerStyle;
}
