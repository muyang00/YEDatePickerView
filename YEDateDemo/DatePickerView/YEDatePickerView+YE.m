//
//  YEDatePickerView+YE.m
//  YEPickerView
//
//  Created by yongen on 20/6/13.
...
YESYNTH_DUMMY_CLASS(YEDatePickerView_YE)

//////////////////////////////////////////
/// 本分类主要是给 YEDatePickerView 文件瘦身
//////////////////////////////////////////

@implementation YEDatePickerView (YE)

#pragma mark - 最小日期
- (NSDate *)handlerMinDate:(NSDate *)minDate {
    if (!minDate) {
        if (self.pickerMode == YEDatePickerModeMDHM) {
            minDate = [NSDate ye_setMonth:1 day:1 hour:0 minute:0];
        } else if (self.pickerMode == YEDatePickerModeMD) {
            minDate = [NSDate ye_setMonth:1 day:1];
        } else if (self.pickerMode == YEDatePickerModeTime || self.pickerMode == YEDatePickerModeCountDownTimer || self.pickerMode == YEDatePickerModeHM) {
            minDate = [NSDate ye_setHour:0 minute:0];
        } else if (self.pickerMode == YEDatePickerModeHMS) {
            minDate = [NSDate ye_setHour:0 minute:0 second:0];
        } else if (self.pickerMode == YEDatePickerModeMS) {
            minDate = [NSDate ye_setMinute:0 second:0];
        } else {
            minDate = [NSDate distantPast]; // 遥远的过去的一个时间点
            minDate = [NSDate dateWithTimeIntervalSince1970:0];
        }
    }
    return minDate;
}
...
- (NSDate *)handlerMaxDate:(NSDate *)maxDate {
    if (!maxDate) {
        if (self.pickerMode == YEDatePickerModeMDHM) {
            maxDate = [NSDate ye_setMonth:12 day:31 hour:23 minute:59];
        } else if (self.pickerMode == YEDatePickerModeMD) {
            maxDate = [NSDate ye_setMonth:12 day:31];
        } else if (self.pickerMode == YEDatePickerModeTime || self.pickerMode == YEDatePickerModeCountDownTimer || self.pickerMode == YEDatePickerModeHM) {
            maxDate = [NSDate ye_setHour:23 minute:59];
        } else if (self.pickerMode == YEDatePickerModeHMS) {
            maxDate = [NSDate ye_setHour:23 minute:59 second:59];
        } else if (self.pickerMode == YEDatePickerModeMS) {
            maxDate = [NSDate ye_setMinute:59 second:59];
        } else {
            maxDate = [NSDate distantFuture]; // 遥远的未来的一个时间点
            maxDate = [NSDate ye_dateFromString:@"2049-12-31 23:59:59" dateFormat:@"yyyy-MM-dd HH:mm:ss"];
        }
    }
    return maxDate;
}
...
- (NSDate *)handlerSelectDate:(NSDate *)selectDate dateFormat:(NSString *)dateFormat {
    ...
    if (!selectDate) {
        if (self.selectValue && self.selectValue.length > 0) {
            ...
            if ([self.selectValue isEqualToString:self.lastRowContent]) {
                ...
                date = [self ye_dateFromString:self.selectValue dateFormat:dateFormat];
                if (!date) {
                    YEErrorLog(@"参数异常！字符串 selectValue 的正确格式是：%@", dateFormat);
                    NSAssert(date, @"参数异常！请检查字符串 selectValue 的格式");
                    date = [NSDate date]; // 默认值参数格式错误时，重置/忽略默认值，防止在 Release 环境下崩溃！
                }
                if (self.pickerMode == YEDatePickerModeMDHM) {
                    selectDate = [NSDate ye_setMonth:date.ye_month day:date.ye_day hour:date.ye_hour minute:date.ye_minute];
                } else if (self.pickerMode == YEDatePickerModeMD) {
                    selectDate = [NSDate ye_setMonth:date.ye_month day:date.ye_day];
                } else if (self.pickerMode == YEDatePickerModeTime || self.pickerMode == YEDatePickerModeCountDownTimer || self.pickerMode == YEDatePickerModeHM) {
                    selectDate = [NSDate ye_setHour:date.ye_hour minute:date.ye_minute];
                } else if (self.pickerMode == YEDatePickerModeHMS) {
                    selectDate = [NSDate ye_setHour:date.ye_hour minute:date.ye_minute second:date.ye_second];
                } else if (self.pickerMode == YEDatePickerModeMS) {
                    selectDate = [NSDate ye_setMinute:date.ye_minute second:date.ye_second];
                } else {
                    selectDate = date;
                }
            }
            ...
            // 不设置默认日期
            if (self.pickerMode == YEDatePickerModeTime ||
                self.pickerMode == YEDatePickerModeCountDownTimer ||
                self.pickerMode == YEDatePickerModeHM ||
                self.pickerMode == YEDatePickerModeHMS ||
                self.pickerMode == YEDatePickerModeMS) {
                // 默认选中最小日期
                selectDate = self.minDate;
            } else {
                if (self.minuteInterval > 1 || self.secondInterval > 1) {
                    NSDate *date = [NSDate date];
                    NSInteger minute = self.minDate.ye_minute % self.minuteInterval == 0 ? self.minDate.ye_minute : 0;
                    NSInteger second = self.minDate.ye_second % self.secondInterval == 0 ? self.minDate.ye_second : 0;
                    selectDate = [NSDate ye_setYear:date.ye_year month:date.ye_month day:date.ye_day hour:date.ye_hour minute:minute second:second];
                } else {
                    // 默认选中今天的日期
    ...
    // 判断日期是否超过边界限制
    BOOL selectLessThanMin = [self ye_compareDate:selectDate targetDate:self.minDate dateFormat:dateFormat] == NSOrderedAscending;
    BOOL selectMoreThanMax = [self ye_compareDate:selectDate targetDate:self.maxDate dateFormat:dateFormat] == NSOrderedDescending;
    if (selectLessThanMin) {
        YEErrorLog(@"默认选择的日期不能小于最小日期！");
        selectDate = self.minDate;
    }
    if (selectMoreThanMax) {
        YEErrorLog(@"默认选择的日期不能大于最大日期！");
...
- (NSString *)ye_stringFromDate:(NSDate *)date dateFormat:(NSString *)dateFormat {
    return [NSDate ye_stringFromDate:date dateFormat:dateFormat timeZone:self.timeZone language:self.pickerStyle.language];
}

#pragma mark - NSString 转 NSDate
- (NSDate *)ye_dateFromString:(NSString *)dateString dateFormat:(NSString *)dateFormat {
    return [NSDate ye_dateFromString:dateString dateFormat:dateFormat timeZone:self.timeZone language:self.pickerStyle.language];
}

#pragma mark - 比较两个日期大小（可以指定比较级数，即按指定格式进行比较）
- (NSComparisonResult)ye_compareDate:(NSDate *)date targetDate:(NSDate *)targetDate dateFormat:(NSString *)dateFormat {
    NSString *dateString1 = [self ye_stringFromDate:date dateFormat:dateFormat];
    NSString *dateString2 = [self ye_stringFromDate:targetDate dateFormat:dateFormat];
    NSDate *date1 = [self ye_dateFromString:dateString1 dateFormat:dateFormat];
    NSDate *date2 = [self ye_dateFromString:dateString2 dateFormat:dateFormat];
...
- (NSArray *)getYearArr {
    ...
    for (NSInteger i = self.minDate.ye_year; i <= self.maxDate.ye_year; i++) {
...
- (NSArray *)getMonthArr:(NSInteger)year {
    ...
    if (year == self.minDate.ye_year) {
        startMonth = self.minDate.ye_month;
    }
    if (year == self.maxDate.ye_year) {
        endMonth = self.maxDate.ye_month;
    }
    NSMutableArray *tempArr = [[NSMutableArray alloc]init];
...
- (NSArray *)getDayArr:(NSInteger)year month:(NSInteger)month {
    ...
    NSInteger endDay = [NSDate ye_getDaysInYear:year month:month];
    if (year == self.minDate.ye_year && month == self.minDate.ye_month) {
        startDay = self.minDate.ye_day;
    }
    if (year == self.maxDate.ye_year && month == self.maxDate.ye_month) {
        endDay = self.maxDate.ye_day;
    }
    NSMutableArray *tempArr = [[NSMutableArray alloc]init];
...
- (NSArray *)getHourArr:(NSInteger)year month:(NSInteger)month day:(NSInteger)day {
    ...
    if (year == self.minDate.ye_year && month == self.minDate.ye_month && day == self.minDate.ye_day) {
        startHour = self.minDate.ye_hour;
    }
    if (year == self.maxDate.ye_year && month == self.maxDate.ye_month && day == self.maxDate.ye_day) {
        endHour = self.maxDate.ye_hour;
    }
    NSMutableArray *tempArr = [[NSMutableArray alloc]init];
...
- (NSArray *)getMinuteArr:(NSInteger)year month:(NSInteger)month day:(NSInteger)day hour:(NSInteger)hour {
    ...
    if (year == self.minDate.ye_year && month == self.minDate.ye_month && day == self.minDate.ye_day && hour == self.minDate.ye_hour) {
        startMinute = self.minDate.ye_minute;
    }
    if (year == self.maxDate.ye_year && month == self.maxDate.ye_month && day == self.maxDate.ye_day && hour == self.maxDate.ye_hour) {
        endMinute = self.maxDate.ye_minute;
    }
    NSMutableArray *tempArr = [[NSMutableArray alloc]init];
...
- (NSArray *)getSecondArr:(NSInteger)year month:(NSInteger)month day:(NSInteger)day hour:(NSInteger)hour minute:(NSInteger)minute {
    ...
    if (year == self.minDate.ye_year && month == self.minDate.ye_month && day == self.minDate.ye_day && hour == self.minDate.ye_hour && minute == self.minDate.ye_minute) {
        startSecond = self.minDate.ye_second;
    }
    if (year == self.maxDate.ye_year && month == self.maxDate.ye_month && day == self.maxDate.ye_day && hour == self.maxDate.ye_hour && minute == self.maxDate.ye_minute) {
        endSecond = self.maxDate.ye_second;
    }
    NSMutableArray *tempArr = [[NSMutableArray alloc]init];
...
- (NSArray *)getMonthWeekArr:(NSInteger)year month:(NSInteger)month {
    ...
    NSInteger endWeek = [NSDate ye_getWeeksOfMonthInYear:year month:month];
    if (year == self.minDate.ye_year && month == self.minDate.ye_month) {
        startWeek = self.minDate.ye_monthWeek;
    }
    if (year == self.maxDate.ye_year && month == self.maxDate.ye_month) {
        endWeek = self.maxDate.ye_monthWeek;
    }
    NSMutableArray *tempArr = [[NSMutableArray alloc]init];
...
- (NSArray *)getYearWeekArr:(NSInteger)year {
    NSInteger startWeek = 1;
    NSInteger endWeek = [NSDate ye_getWeeksOfYearInYear:year];
    if (year == self.minDate.ye_year) {
        startWeek = self.minDate.ye_yearWeek;
    }
    if (year == self.maxDate.ye_year) {
        endWeek = self.maxDate.ye_yearWeek;
    }
    NSMutableArray *tempArr = [[NSMutableArray alloc]init];
    for (NSInteger i = startWeek; i <= endWeek; i++) {
        [tempArr addObject:[self getMDHMSNumber:i]];
...
- (NSArray *)getQuarterArr:(NSInteger)year {
    ...
    NSInteger endQuarter = [NSDate ye_getQuartersInYear:year];
    if (year == self.minDate.ye_year) {
        startQuarter = self.minDate.ye_quarter;
    }
    if (year == self.maxDate.ye_year) {
        endQuarter = self.maxDate.ye_quarter;
    }
    NSMutableArray *tempArr = [[NSMutableArray alloc]init];
...
- (NSString *)getYearText:(NSArray *)yearArr row:(NSInteger)row {
    ...
    NSString *yearUnit = self.showUnitType == YEShowUnitTypeAll ? [self getYearUnit] : @"";
...
- (NSString *)getMonthText:(NSArray *)monthArr row:(NSInteger)row {
    ...
    if (self.monthNames && self.monthNames.count > 0) {
        ...
        if (![self.pickerStyle.language hasPrefix:@"zh"] && (self.pickerMode == YEDatePickerModeYMD || self.pickerMode == YEDatePickerModeYM || self.pickerMode == YEDatePickerModeYMW)) {
            // 非中文环境：月份使用系统的月份名称
            NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
            dateFormatter.locale = [[NSLocale alloc]initWithLocaleIdentifier:self.pickerStyle.language];
            // monthSymbols: @[@"January", @"FeYEuary", @"March", @"April", @"May", @"June", @"July", @"August", @"September", @"October", @"November", @"December"];
            ...
            // 中文环境：月份显示数字
            NSString *monthUnit = self.showUnitType == YEShowUnitTypeAll ? [self getMonthUnit] : @"";
...
- (NSString *)getDayText:(NSArray *)dayArr row:(NSInteger)row mSelectDate:(NSDate *)mSelectDate {
    ...
    if (self.isShowToday && mSelectDate.ye_year == [NSDate date].ye_year && mSelectDate.ye_month == [NSDate date].ye_month && [dayString integerValue] == [NSDate date].ye_day) {
        return [NSBundle ye_localizedStringForKey:@"今天" language:self.pickerStyle.language];
    }
    NSString *dayUnit = self.showUnitType == YEShowUnitTypeAll ? [self getDayUnit] : @"";
    dayString = [NSString stringWithFormat:@"%@%@", dayString, dayUnit];
    if (self.isShowWeek) {
        NSDate *date = [NSDate ye_setYear:mSelectDate.ye_year month:mSelectDate.ye_month day:[dayString integerValue]];
        NSString *weekdayString = [NSBundle ye_localizedStringForKey:[date ye_weekdayString] language:self.pickerStyle.language];
...
- (NSString *)getHourText:(NSArray *)hourArr row:(NSInteger)row {
    ...
    NSString *hourUnit = self.showUnitType == YEShowUnitTypeAll ? [self getHourUnit] : @"";
...
- (NSString *)getMinuteText:(NSArray *)minuteArr row:(NSInteger)row {
    ...
    NSString *minuteUnit = self.showUnitType == YEShowUnitTypeAll ? [self getMinuteUnit] : @"";
...
- (NSString *)getSecondText:(NSArray *)secondArr row:(NSInteger)row {
    ...
    NSString *secondUnit = self.showUnitType == YEShowUnitTypeAll ? [self getSecondUnit] : @"";
...
- (NSString *)getWeekText:(NSArray *)weekArr row:(NSInteger)row {
    ...
    NSString *weekUnit = self.showUnitType == YEShowUnitTypeAll ? [self getWeekUnit] : @"";
...
- (NSString *)getQuarterText:(NSArray *)quarterArr row:(NSInteger)row {
    ...
    NSString *quarterUnit = self.showUnitType == YEShowUnitTypeAll ? [self getQuarterUnit] : @"";
    return [NSString stringWithFormat:@"%@%@", quarterString, quarterUnit];
}

- (NSString *)getAMText {
    return [NSBundle ye_localizedStringForKey:@"上午" language:self.pickerStyle.language];
}

- (NSString *)getPMText {
    return [NSBundle ye_localizedStringForKey:@"下午" language:self.pickerStyle.language];
}
...
- (NSString *)getYearUnit {
    ...
    return [NSBundle ye_localizedStringForKey:@"年" language:self.pickerStyle.language];
}
...
- (NSString *)getMonthUnit {
    ...
    return [NSBundle ye_localizedStringForKey:@"月" language:self.pickerStyle.language];
}
...
- (NSString *)getDayUnit {
    ...
    return [NSBundle ye_localizedStringForKey:@"日" language:self.pickerStyle.language];
}
...
- (NSString *)getHourUnit {
    ...
    return [NSBundle ye_localizedStringForKey:@"时" language:self.pickerStyle.language];
}
...
- (NSString *)getMinuteUnit {
    ...
    return [NSBundle ye_localizedStringForKey:@"分" language:self.pickerStyle.language];
}
...
- (NSString *)getSecondUnit {
    ...
    return [NSBundle ye_localizedStringForKey:@"秒" language:self.pickerStyle.language];
}
...
- (NSString *)getWeekUnit {
    ...
    return [NSBundle ye_localizedStringForKey:@"周" language:self.pickerStyle.language];
}
...
- (NSString *)getQuarterUnit {
    ...
    return [NSBundle ye_localizedStringForKey:@"季度" language:self.pickerStyle.language];
}
