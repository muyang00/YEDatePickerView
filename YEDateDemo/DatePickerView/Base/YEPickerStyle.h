...
// 边框样式（左边取消按钮/右边确定按钮）
typedef NS_ENUM(NSInteger, YEBorderStyle) {
    /** 无边框（默认） */
    YEBorderStyleNone = 0,
    /** 有圆角和边框 */
    YEBorderStyleSolid,
...
@interface YEPickerStyle : NSObject
...
/** 设置 cancelBtn 的边框样式 */
@property (nonatomic, assign) YEBorderStyle cancelBorderStyle;
...
/** 设置 doneBtn 的边框样式 */
@property (nonatomic, assign) YEBorderStyle doneBorderStyle;
...
/////// 日期选择器单位样式（showUnitType == YEShowUnitTypeOnlyCenter 时生效。暂不支持日期选择器前4种类型 ）///////
