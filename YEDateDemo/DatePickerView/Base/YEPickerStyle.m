...
//  YEPickerView
//
//  Created by yongen on 19/10/20.
//  Copyright © 2017年 yongen. All rights reserved.
//

#import "YEPickerStyle.h"
#import "NSBundle+YEPickerView.h"

// 标题颜色
#define kYEDefaultTextColor YE_RGB_HEX(0x333333, 1.0f)

@implementation YEPickerStyle
...
- (UIColor *)maskColor {
    if (!_maskColor) {
        _maskColor = [self ye_colorWithLightColor:YE_RGB_HEX(0x000000, 0.3f) darkColor:YE_RGB_HEX(0x666666, 0.3f)];
    }
    return _maskColor;
}
...
- (UIColor *)shadowLineColor {
    if (!_shadowLineColor) {
        if (@available(iOS 13.0, *)) {
            ...
            _shadowLineColor = YE_RGB_HEX(0xc6c6c8, 1.0f);
        }
    }
...
- (CGFloat)paddingBottom {
    if (_paddingBottom <= 0) {
        _paddingBottom = YE_BOTTOM_MARGIN;
    }
...
- (UIColor *)titleLineColor {
    if (!_titleLineColor) {
        _titleLineColor = [self ye_colorWithLightColor:YE_RGB_HEX(0xededee, 1.0f) darkColor:YE_RGB_HEX(0x18181c, 1.0f)];
    }
...
- (UIColor *)cancelTextColor {
    if (!_cancelTextColor) {
        if (@available(iOS 13.0, *)) {
            ...
            _cancelTextColor = kYEDefaultTextColor;
        }
    }
...
- (NSString *)cancelBtnTitle {
    if (!_cancelBtnTitle && !_cancelBtnImage) {
        _cancelBtnTitle = [NSBundle ye_localizedStringForKey:@"取消" language:self.language];
    }
...
- (UIColor *)titleTextColor {
    if (!_titleTextColor) {
        if (@available(iOS 13.0, *)) {
            ...
            _titleTextColor = YE_RGB_HEX(0x999999, 1.0f);
        }
    }
...
- (CGRect)titleLabelFrame {
    if (CGRectEqualToRect(_titleLabelFrame, CGRectZero) || _titleLabelFrame.size.height == 0) {
        _titleLabelFrame = CGRectMake(5 + 60 + 2, 0, YEGetKeyWindow().bounds.size.width - 2 * (5 + 60 + 2), 44);
    }
...
- (UIColor *)doneTextColor {
    if (!_doneTextColor) {
        if (@available(iOS 13.0, *)) {
            ...
            _doneTextColor = kYEDefaultTextColor;
        }
    }
...
- (NSString *)doneBtnTitle {
    if (!_doneBtnTitle && !_doneBtnImage) {
        _doneBtnTitle = [NSBundle ye_localizedStringForKey:@"确定" language:self.language];
    }
...
- (CGRect)doneBtnFrame {
    if (CGRectEqualToRect(_doneBtnFrame, CGRectZero) || _doneBtnFrame.size.height == 0) {
        _doneBtnFrame = CGRectMake(YEGetKeyWindow().bounds.size.width - 60 - 5, 8, 60, 28);
    }
...
- (UIColor *)separatorColor {
    if (!_separatorColor) {
        if (@available(iOS 13.0, *)) {
            ...
            _separatorColor = YE_RGB_HEX(0xc6c6c8, 1.0f);
        }
    }
...
- (UIColor *)pickerTextColor {
    if (!_pickerTextColor) {
        if (@available(iOS 13.0, *)) {
            ...
            _pickerTextColor = kYEDefaultTextColor;
        }
    }
...
- (UIColor *)dateUnitTextColor {
    if (!_dateUnitTextColor) {
        if (@available(iOS 13.0, *)) {
            ...
            _dateUnitTextColor = kYEDefaultTextColor;
        }
    }
...
- (UIColor *)ye_colorWithLightColor:(UIColor *)lightColor darkColor:(UIColor *)darkColor {
    if (@available(iOS 13.0, *)) {
...
+ (instancetype)pickerStyleWithThemeColor:(UIColor *)themeColor {
    YEPickerStyle *customStyle = [[self alloc]init];
    if (themeColor) {
        customStyle.cancelTextColor = themeColor;
        customStyle.cancelBorderStyle = YEBorderStyleSolid;
        customStyle.doneColor = themeColor;
        customStyle.doneTextColor = [UIColor whiteColor];
        customStyle.doneBorderStyle = YEBorderStyleFill;
    }
    return customStyle;
}
...
+ (instancetype)pickerStyleWithDoneTextColor:(UIColor *)doneTextColor {
    ...
    if (doneTextColor) {
        ...
        customStyle.doneBtnFrame = CGRectMake(YEGetKeyWindow().bounds.size.width - 60, 4, 60, 40);
        customStyle.doneBtnTitle = [NSBundle ye_localizedStringForKey:@"完成" language:customStyle.language];
...
+ (instancetype)pickerStyleWithDoneBtnImage:(UIImage *)doneBtnImage {
    ...
    if (doneBtnImage) {
        ...
        customStyle.doneBtnFrame = CGRectMake(YEGetKeyWindow().bounds.size.width - 44, 4, 40, 40);
    }
    return customStyle;
}
...
+ (void)ye_setView:(UIView *)view roundingCorners:(UIRectCorner)corners withRadius:(CGFloat)radius {
