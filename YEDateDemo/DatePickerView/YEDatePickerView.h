...
/// 日期选择器格式
typedef NS_ENUM(NSInteger, YEDatePickerMode) {
    // ----- 以下4种是系统样式（兼容国际化日期格式） -----
    /** 【yyyy-MM-dd】UIDatePickerModeDate（美式日期：MM-dd-yyyy；英式日期：dd-MM-yyyy）*/
    YEDatePickerModeDate,
    /** 【yyyy-MM-dd HH:mm】 UIDatePickerModeDateAndTime */
    YEDatePickerModeDateAndTime,
    /** 【HH:mm】UIDatePickerModeTime */
    YEDatePickerModeTime,
    /** 【HH:mm】UIDatePickerModeCountDownTimer */
    YEDatePickerModeCountDownTimer,
    
    // ----- 以下14种是自定义样式 -----
    /** 【yyyy-MM-dd HH:mm:ss】年月日时分秒 */
    YEDatePickerModeYMDHMS,
    /** 【yyyy-MM-dd HH:mm】年月日时分 */
    YEDatePickerModeYMDHM,
    /** 【yyyy-MM-dd HH】年月日时 */
    YEDatePickerModeYMDH,
    /** 【MM-dd HH:mm】月日时分 */
    YEDatePickerModeMDHM,
...
/// 日期单位显示的位置
typedef NS_ENUM(NSInteger, YEShowUnitType) {
    /** 日期单位显示全部行（默认）*/
    YEShowUnitTypeAll,
    /** 日期单位仅显示中间行 */
    YEShowUnitTypeOnlyCenter,
    /** 日期单位不显示（隐藏日期单位）*/
    YEShowUnitTypeNone
};

typedef void (^YEDateResultBlock)(NSDate * _Nullable selectDate, NSString * _Nullable selectValue);

typedef void (^YEDateResultRangeBlock)(NSDate * _Nullable selectStartDate, NSDate * _Nullable selectEndDate, NSString * _Nullable selectValue);

@interface YEDatePickerView : YEPickerBaseView
...
/** 日期选择器显示类型 */
@property (nonatomic, assign) YEDatePickerMode pickerMode;
...
/** 最小日期（可使用 NSDate+YEPickerView 分类中对应的方法进行创建）*/
@property (nullable, nonatomic, strong) NSDate *minDate;
/** 最大日期（可使用 NSDate+YEPickerView 分类中对应的方法进行创建）*/
@property (nullable, nonatomic, strong) NSDate *maxDate;

/** 选择结果的回调 */
@property (nullable, nonatomic, copy) YEDateResultBlock resultBlock;
/** 选择结果范围的回调：for `YEDatePickerModeYQ`、`YEDatePickerModeYMW`、`YEDatePickerModeYW`, ignored otherwise. */
@property (nullable, nonatomic, copy) YEDateResultRangeBlock resultRangeBlock;

/** 滚动选择时触发的回调 */
@property (nullable, nonatomic, copy) YEDateResultBlock changeBlock;
/** 滚动选择范围时触发的回调：for `YEDatePickerModeYQ`、`YEDatePickerModeYMW`、`YEDatePickerModeYW`, ignored otherwise. */
@property (nullable, nonatomic, copy) YEDateResultRangeBlock changeRangeBlock;

/** 日期单位显示类型 */
@property (nonatomic, assign) YEShowUnitType showUnitType;
...
/** 设置倒计时的时长，默认为0（范围：0 ~ 24*60*60-1，单位为秒） for `YEDatePickerModeCountDownTimer`, ignored otherwise. */
...
/**
 *  设置国际化日期(非中文环境下)月份是否显示简称，默认为 NO。for `YEDatePickerModeYMD` and `YEDatePickerModeYM`, ignored otherwise.
 *  如：January 的简称为：Jan
 */
...
/** 显示上午和下午，默认为 NO. for `YEDatePickerModeYMDH`, ignored otherwise. */
...
/** 不允许选择日期的回调 */
@property (nullable, nonatomic, copy) YEDateResultBlock nonSelectableBlock;
...
+ (void)showDatePickerWithMode:(YEDatePickerMode)mode
                   ...
                   resultBlock:(nullable YEDateResultBlock)resultBlock;
 ...
 *  @param title            选择器标题
 *  @param selectValue      默认选中的日期（默认选中当前日期）
 *  @param minDate          最小日期（可使用 NSDate+YEPickerView 分类中对应的方法进行创建）
 *  @param maxDate          最大日期（可使用 NSDate+YEPickerView 分类中对应的方法进行创建）
 *  @param isAutoSelect     是否自动选择，即滚动选择器后就执行结果回调，默认为 NO
 *  @param resultBlock      选择结果的回调
 *
 */
...
+ (void)showDatePickerWithMode:(YEDatePickerMode)mode
                  ...
                  isAutoSelect:(BOOL)isAutoSelect
                   resultBlock:(nullable YEDateResultBlock)resultBlock
              resultRangeBlock:(nullable YEDateResultRangeBlock)resultRangeBlock;
